...
def bsp_update_sconstruct(dist_dir):
    with open(os.path.join(dist_dir, 'SConstruct'), 'r', encoding='utf-8') as f:
        data = f.readlines()
    with open(os.path.join(dist_dir, 'SConstruct'), 'w', encoding='utf-8') as f:
        for line in data:
...
def bsp_update_kconfig_testcases(dist_dir):
    ...
    with open(os.path.join(dist_dir, 'rt-thread/Kconfig'), 'r', encoding='utf-8') as f:
        data = f.readlines()
